...
const App: React.FC = () => {
  ...
  // Global Asset State (Initialized with Mock Data)
  ...
  const handleAvatarClick = () => {
    ...
    handleProfileUpdate('avatarUrl', `https://picsum.photos/seed/${seed}/200`);
  };
        ...
        setAssets(prev => prev.map(asset => {
            if (asset.id === editingAssetId) {
                return {
                    ...
                    totalValue: Number(newAsset.totalValue),
                    fractionPrice: Number(newAsset.fractionPrice),
                } as ArtAsset;
            }
            return asset;
        }));
        showNotification('Dados do ativo atualizados!');
    } else {
        // CREATE New Asset
        const assetToAdd: ArtAsset = {
            id: `new-${Date.now()}`,
            title: newAsset.title!,
            artist: newAsset.artist!,
            year: Number(newAsset.year) || 2024,
    ...
    <div className="p-6 pb-32 space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        ...
        <button 
          ...
          className="h-10 w-10 bg-slate-800 rounded-full flex items-center justify-center border border-slate-700 shadow-lg active:scale-90 transition-transform hover:border-amber-500/50 cursor-pointer group overflow-hidden"
        >
  ...
  const renderProfile = () => (
    <div className="p-6 pb-32 space-y-8 animate-in slide-in-from-right duration-300 bg-slate-950 min-h-screen relative">
      ...
      <div className="flex flex-col items-center gap-4 py-6">
        ...
        >
          <div className="w-28 h-28 rounded-full bg-slate-800 border-2 border-amber-500/50 flex items-center justify-center overflow-hidden shadow-2xl group-hover:border-amber-500 transition-colors">
            {userProfile.avatarUrl ? (
                <img src={userProfile.avatarUrl} className="w-full h-full object-cover" alt="User Avatar" />
            ) : (
                <i className="fa-solid fa-user text-5xl text-slate-600 group-hover:text-slate-400"></i>
            )}
          </div>
          <div className="absolute bottom-0 right-0 bg-amber-500 h-8 w-8 rounded-full flex items-center justify-center text-slate-900 border border-slate-950 shadow-lg group-hover:scale-110 transition-transform">
      ...
      {userProfile.isAdmin && (
        <button 
          ...
          className="w-full bg-slate-800 border-2 border-slate-700 text-slate-300 font-black py-4 rounded-xl active:scale-95 transition-all shadow-lg text-xs uppercase tracking-[0.2em] hover:bg-slate-700 hover:text-white hover:border-amber-500/50 flex items-center justify-center gap-3"
        >
  ...
  const renderAdmin = () => (
    <div className="p-6 pb-32 space-y-6 animate-in slide-in-from-bottom duration-500 bg-slate-950 min-h-screen">
      ...
      {/* Lista de Ativos Existentes para Edição */}
      <div className="mb-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-6 shadow-2xl relative">
         ...
         <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Título da Obra</label>
                <input 
                    type="text" 
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                    ...
                    <input 
                        type="number" 
                        value={newAsset.year}
                        onChange={(e) => setNewAsset(prev => ({ ...prev, year: parseInt(e.target.value) }))}
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white font-bold focus:border-amber-500 focus:outline-none transition-colors"
                    />
                </div>
                <div className="space-y-1">
            ...
            <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Imagem da Obra</label>
            ...
            <div className="pt-4 border-t border-slate-800 space-y-4">
                <h4 className="text-slate-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                    <i className="fa-solid fa-shield-halved text-amber-500"></i> Dados da Apólice
                </h4>
            ...
            <div className="flex gap-3 mt-4">
                ...
                <button 
                    ...
                    className={`flex-[2] font-black py-4 rounded-xl shadow-lg active:scale-95 transition-all text-xs uppercase tracking-[0.2em] ${editingAssetId ? 'bg-amber-500 text-slate-900 shadow-amber-500/20 hover:bg-amber-400' : 'bg-emerald-600 text-white shadow-emerald-900/20 hover:bg-emerald-500'}`}
                >
                    {editingAssetId ? 'Salvar Alterações' : 'Confirmar Inserção'}
                </button>
            </div>
         </div>
      </div>
  ...
  const renderCatalog = () => (
    <div className="p-6 pb-32 space-y-8 animate-in slide-in-from-bottom duration-700 bg-slate-950">
      ...
      <div className="space-y-12">
        {assets.filter(a => a.isCatalogOnly).map(asset => (
          <div 
            key={asset.id}
            onClick={() => navigateToAsset(asset)}
